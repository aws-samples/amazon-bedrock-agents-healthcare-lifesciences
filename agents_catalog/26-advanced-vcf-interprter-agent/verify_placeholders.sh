...
for placeholder in "${PLACEHOLDERS[@]}"; do
    ...
    # Search for the placeholder in relevant files, excluding documentation files
    MATCHES=$(find . -type f \( -name "*.py" -o -name "*.ipynb" -o -name "*.md" \) ! -name "CONFIGURATION_PLACEHOLDERS.md" ! -name "verify_placeholders.sh" ! -name "FILE_INVENTORY.md" ! -name "STEP_BY_STEP_GUIDE.md" -exec grep -l "$placeholder" {} \; 2>/dev/null)
