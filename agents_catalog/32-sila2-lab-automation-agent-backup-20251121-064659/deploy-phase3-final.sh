...
RUNTIME_STATUS_OUTPUT=$(agentcore status -a sila2_runtime_phase3_simple 2>/dev/null || echo "Runtime not found")
...
if echo "$GATEWAY_LIST" | grep -q "sila2-gateway-phase3"; then
    ...
    if [ "$RUNTIME_STATUS" = "deployed" ]; then
        ...
        GATEWAY_RESULT=$(agentcore create_mcp_gateway --name sila2-gateway-phase3 --agent sila2_runtime_phase3_simple 2>&1 || echo "Gateway creation failed")
...
echo "   agentcore invoke '{\"message\": \"Hello\"}' -a sila2_runtime_phase3_simple"
echo ""
