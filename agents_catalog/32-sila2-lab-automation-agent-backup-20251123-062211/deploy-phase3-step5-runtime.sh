#!/bin/bash
# Phase 3 Step 5: AgentCore Runtime デプロイ (シンプル版)

set -e
source .phase3-config

echo "🤖 Phase 3 Step 5: AgentCore Runtime デプロイ (シンプル版)"
echo "📍 リージョン: $REGION"
echo "📍 エージェント名: sila2_runtime_phase3_simple"
...
# Step 9: AgentCore configure実行 (シンプル版)
echo "🔧 AgentCore configure実行中 (シンプル版)..."
...
(echo "requirements.txt"; echo "no") | ~/.pyenv/versions/3.10.*/bin/agentcore configure \
  ...
  --ecr "$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/bedrock-agentcore-sila2_runtime_phase3_simple" \
  --region $REGION

echo "✅ AgentCore設定完了 (シンプル版)"

# Step 10: AgentCoreデプロイ (CodeBuildモード)
...
# Step 12: デプロイ結果表示
echo "🎉 Phase 3 Step 5 完了: AgentCore Runtime デプロイ成功 (シンプル版)"
echo ""
echo "📋 デプロイ結果:"
...
echo "   python test_phase3_simple.py"