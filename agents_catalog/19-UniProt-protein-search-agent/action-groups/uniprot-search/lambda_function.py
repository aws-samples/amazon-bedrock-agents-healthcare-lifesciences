...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if not query:
            return create_response(event, "Error: Query parameter is required for protein search.")
        
        logger.info(f"Searching for proteins: query='{query}', organism='{organism}', limit={limit}")
...
def format_search_results(data: Dict[str, Any], query: str) -> str:
    ...
    for i, protein in enumerate(results, 1):
        ...
        gene_name = gene_names[0].get('geneName', {}).get('value', 'N/A') if gene_names else 'N/A'
        ...
        for comment in function_comments:
            if comment.get('commentType') == 'FUNCTION':
                ...
                if texts:
                    function_desc = texts[0].get('value', 'N/A')[:200] + '...' if len(texts[0].get('value', '')) > 200 else texts[0].get('value', 'N/A')
                break
