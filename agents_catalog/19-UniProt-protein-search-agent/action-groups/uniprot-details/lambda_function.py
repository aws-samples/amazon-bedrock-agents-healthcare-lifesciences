...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if not validate_accession_id(accession_id):
            return create_response(event, f"Error: Invalid UniProt accession ID format: {accession_id}. Expected format: 6-10 alphanumeric characters (e.g., P04637)")
        
        logger.info(f"Retrieving protein details: accession_id='{accession_id}', include_sequence={include_sequence}, include_features={include_features}")
...
def get_protein_details(accession_id: str, include_sequence: bool, include_features: bool) -> str:
    """
    ...
    try:
        ...
        # Format the results
        return format_protein_details(data, accession_id, include_sequence, include_features)
...
def format_protein_details(data: Dict[str, Any], accession_id: str, include_sequence: bool, include_features: bool) -> str:
    """
    ...
    try:
        ...
        primary_gene = gene_names[0].get('geneName', {}).get('value', 'N/A') if gene_names else 'N/A'
    ...
    for comment in comments:
        if comment.get('commentType') == 'DISEASE':
            ...
            if disease:
                ...
                if description:
                    desc_text = description[0].get('value', '')[:200] + '...' if len(description[0].get('value', '')) > 200 else description[0].get('value', '')
