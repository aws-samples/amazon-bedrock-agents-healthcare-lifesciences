...
# Sets ICH_MCP_TOKEN and ICH_MCP_URL environment variables
...
# Get OAuth2 token
export ICH_MCP_TOKEN=$(curl -s -X POST "$TOKEN_URL" \
...
export ICH_MCP_URL=$(aws ssm get-parameter --name "/app/$APP_NAME/agentcore/mcp_url" --query "Parameter.Value" --output text --region "$REGION" 2>/dev/null || echo "NOT_YET_CONFIGURED")

echo "ICH_MCP_TOKEN set (${#ICH_MCP_TOKEN} chars)"
echo "ICH_MCP_URL=$ICH_MCP_URL"
